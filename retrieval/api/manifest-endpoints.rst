...
The manifest endpoints are used to manage and retrieve information about
sample manifests, including fetching manifests, sample records and
validation reports.
...
.. tip::
   ...
   * To view details of the endpoint, click the
     |manifest-collapsible-item-arrow| *Show endpoint details* button.

   * Then, inside that section, click the |manifest-collapsible-item-arrow|
...
.. collapse:: Show endpoint details
   ...
   **Usage**
    ...
    * This endpoint returns a list of manifest :abbr:`IDs (Identifications)`
      of sample records.

    * After having retrieved a ``manifest_id`` value, you can use it to
      retrieve full sample information using the
      :ref:`manifest-api-endpoint-sample-record-by-manifest-id` endpoint or
      updates using the
...
.. collapse:: Show endpoint details
   ...
   .. collapse:: Show API query parameters
      ...
      * **sequencing_centre** (required): The name of the sequencing centre
        ...
        `endpoint <sample-api-method-get_sample_by_sequencing_centre_>`__.
   ...
   **Usage**
    ...
    API URL to retrieve a list of manifest IDs records by that sequencing
    centre. Replace ``<sequencing-centre>`` with the desired sequencing centre
    name.
   ...
   **Example**
    ...
    To retrieve manifest IDs associated with the sequencing centre
...
.. collapse:: Show endpoint details
   ...
   .. collapse:: Show API query parameters
      ...
      * **manifest_type** (optional): The type of the manifest [#f2]_. If no
        manifest type is provided, COPO returns a list of project names along
        with the latest manifest version associated with each.
      * **return_type** (optional): Output format for the results. Options
        include **json** (default) and **csv**
   ...
   **Example**
    ...
    To retrieve the current manifest version of the ``DTOL`` project in the
    default JSON format, use the following URL.
...
.. collapse:: Show endpoint details
   ...
   **Usage**
    ...
    Please include at least the ``manifest_id`` parameter value in the API URL
    to retrieve sample records for that manifest. Replace ``{manifest_id}``
    with the desired manifest ID.
   ...
   **Example**
    ...
    To retrieve sample records for the manifest ID
    ``f8e5c23d-f735-439f-bfaf-a6886e31741e`` in the ``rocrate`` format, use
    the following URL.
...
.. collapse:: Show endpoint details
   ...
   **Usage**
    ...
    API URL to retrieve a list of manifest :abbr:`IDs (Identifications)`
    [#f1]_ in a given date period. Replace ``{from}`` and ``{to}`` with the
    ...
    **Example**
    ...
    To retrieve manifest IDs recorded between 1st January, 2025 and 1st May,
...
.. collapse:: Show endpoint details
   ...
   **Usage**
    ...
    Please include at least the ``project``, ``from`` and ``to`` parameter
    values in the API URL to retrieve a list of manifest
    :abbr:`IDs (Identifications)`  [#f1]_  in a given date period for that
    project. Replace ``{project}``, ``{from}`` and ``{to}`` with the desired
    ...
    **Example**
    ...
    To retrieve manifest IDs for the ``ERGA`` project recorded between 1st
    January, 2025 and 1st May, 2025 in the default JSON format, use the
    following URL.
...
.. collapse:: Show endpoint details
   ...
   **Usage**
    ...
    Please include at least the ``manifest_id`` parameter value in the API URL
    to retrieve minimal sample status information for each sample for that
    manifest ID. Replace ``{manifest_id}`` with the desired manifest ID.
   ...
   **Example**
    ...
    To retrieve status information for sample records with the manifest ID
    ``f8e5c23d-f735-439f-bfaf-a6886e31741e`` in the default CSV format, use
    the following URL.
...
.. collapse:: Show endpoint details
   ...
   .. collapse:: Show API query parameters
      ...
      * **profile_id** (required): The identifier of the profile associated
        with the manifest to be validated.
      * **file** (optional): The manifest file to be validated. This should be
        a valid **CSV** or **XLSX** manifest file.
   ...
   **Usage**
    ...
    * Please include at least the ``profile_id`` parameter value in the API
      URL to validate a manifest associated with that profile and receive
      the validation report ID. Replace ``{profile_id}`` with the profile ID.

    * After having retrieved the validation report ID, you can use it to
      retrieve the validation report using the
...
.. collapse:: Show endpoint details
   ...
   .. collapse:: Show API query parameters
      ...
      .. note::
         ...
         Authentication is required in order to use this API method. Create
         an API key from the :ref:`/apiKey API endpoint <endpoints-api-key>`
         before using this method.

      **validation_report_id** (required): The identifier of the validation
      report.
   ...
   **Usage**
    ...
    Please include the ``validation_report_id`` parameter value in the API URL
    to retrieve the manifest validation  report associated with that ID.
...
.. [#f2] Refer to the :ref:`project-affiliations` section for more information
...
.. _sample-api-method-get_sample_by_sequencing_centre: https://copo-project.org/static/swagger/apidocs_index.html#/Sample/get_sample_sequencing_centre
