...
===========================
Finding Data Submission IDs
===========================

Accepted and submitted data are assigned unique identifiers known as
accessions.

Submitted data are publicly visible after publication or release (see
:ref:`publishing-data` for more information). They can be viewed using the
accessions on public repositories (see :ref:`overview-public-repositories`
for more information).

The type of accession assigned depends on the type of submission made. The
following are different kinds of accessions that can be assigned after
...
.. tip::
   ...
   For sample submissions within a Tree of Life profile, the **Approval Date**
   column in the sample data table indicates when the submission was approved
   and which project the approving sample manager belongs to.

   The **Status** column indicates the current status of the submission. Once
   the submission are approved and has been successful, the status will be
   marked as **accepted**.
...
.. note::
   ...
   Please note that the |accept-reject-samples-navigation-button| button will
   only appear on a page if you are granted permission to be a sample manager.
...
On Profile's Accessions Page
----------------------------

Each profile has an accessions page. It provides a consolidated view of all
accessions associated with the profile's submissions.

To access it, refer to :doc:`/profile/components/access-accessions-page` for
instructions.
...
.. _accessions-dashboard: https://copo-project.org/copo/copo_accessions/dashboard
